...
while i < len(lines):
    line = lines[i]
    if "const activeWb = waterBalances.find(wb => wb.id === analyzeBalanceId);" in line and "analyzeSupplyData" in "".join(out[-10:]):
        out.append(line)
        out.append("    const isEstimado = activeWb?.tipoBalanco === 'Estimado';\n")
        i += 1
        continue
    if "const mods = baseDemand.modifiers;" in line and "let totalPopAtendida = 0;" in out[-1]:
        ...
        out.append("               totalDem += (activeWb?.systemDemands?.[`${sys.id}-${year}`] || 0);\n")
        out.append("           });\n")
        out.append("        } else {\n")
