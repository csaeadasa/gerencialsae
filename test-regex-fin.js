...
let preparedText = finText;
// We apply the same regex replace:
...
for (const line of lines) {
  ...
  if (artMatch) {
    ...
    if (!hasRealCaput) {
      ...
      counter++;
      currentSubunit = { id: "caput", type: "caput", label: `Art. ${artMatch[1]}`, text: cleanLine, orderIndex: counter };
    } else {
      if (currentSubunit) subunits.push(currentSubunit);
      counter++;
     ...
     // what if it's text for caput?
     if (currentSubunit && currentSubunit.type === "caput" && subunits.length === 0) {
        currentSubunit.text += "\n" + cleanLine;
     } else if (!currentSubunit) {
        currentSubunit = { id: "intro", type: "outro", label: "Intro", text: cleanLine, orderIndex: ++counter };
     } else {
        currentSubunit.text += "\n" + cleanLine;
     }
  }
...
if (currentSubunit) subunits.push(currentSubunit);
console.log(JSON.stringify(subunits, null, 2));
