...
export function ObrasView() {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-wrap gap-2 rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 to-white p-4 shadow-sm"><input ref={input} type="file" accept=".csv,.xlsx,.xls,.xlsm" className="hidden" onChange={event=>importFile(event.target.files?.[0])}/><label className="relative min-w-[260px] flex-1"><Search className="absolute left-3 top-2.5 text-slate-400" size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Contrato, fornecedor, sistema, local ou situação" className="w-full rounded-xl border border-slate-200 bg-white py-2 pl-9 pr-3 text-sm"/></label><button disabled={busy} onClick={()=>input.current?.click()} className="rounded-xl bg-adasa-mid px-4 py-2 text-sm font-bold text-white shadow-sm hover:bg-blue-700">{busy?<LoaderCircle className="mr-1 inline animate-spin" size={16}/>:<Upload className="mr-1 inline" size={16}/>}Importar CSV</button>{message&&<p className="w-full rounded-lg bg-white px-3 py-2 text-xs font-semibold text-blue-800">{message}</p>}</div>
